...
X_shared = X_shared.sample(frac=0.5, random_state=42)
Y_shared = Y_shared.sample(frac=0.5, random_state=42)
X_shared = X_shared.iloc[:, 5:]
Y_shared = Y_shared.iloc[:, 5]
pd.DataFrame(X_shared).to_csv("./non-IID_data/X_shared.csv", header=False, index=False)
pd.DataFrame(Y_shared).to_csv("./non-IID_data/Y_shared.csv", header=False, index=False)

# client data
X_test = X_test.iloc[:, 5:]
Y_test = Y_test.iloc[:, 5]
pd.DataFrame(X_test).to_csv("./non-IID_data/X_test.csv", header=False, index=False)
pd.DataFrame(Y_test).to_csv("./non-IID_data/Y_test.csv", header=False, index=False)

X_train = X_train.sort_values(["AgeGroup", "Gender"])
X_train = X_train.iloc[:, 5:]
Y_train = Y_train.sort_values(["AgeGroup", "Gender"])
Y_train = Y_train.iloc[:, 5]

pd.DataFrame(X_train).to_csv("./non-IID_data/X_train.csv", header=False, index=False)
pd.DataFrame(Y_train).to_csv("./non-IID_data/Y_train.csv", header=False, index=False)
